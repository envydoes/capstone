...
$adminAccessRevoked = isset($_GET['admin_access']) && $_GET['admin_access'] === 'revoked';
...
<script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script>
  ...
  <style>
    ...
    .badge { display: inline-block; background: color-mix(in srgb, var(--site-primary-light) 15%, transparent); color: var(--site-primary-light); border: 1px solid color-mix(in srgb, var(--site-primary-light) 30%, transparent); font-size: 0.7rem; letter-spacing: 0.12em; text-transform: uppercase; padding: 3px 12px; border-radius: 999px; }
...
<?php endif; ?>
  <header class="w-full h-[68px] border-b border-green-100 flex items-center px-8 bg-white shadow-sm sticky top-0 z-50">
    ...
    <nav class="ml-auto flex items-center gap-4 md:gap-8 text-gray-600 text-sm font-medium">
      ...
      <div class="hidden md:flex gap-8 items-center">
        ...
        <?php if (!$logged_in): ?>
          <a href="login.php" class="px-5 py-2 bg-green-700 hover:bg-green-800 text-white rounded-lg transition text-sm font-semibold shadow">
            Login / Register
          </a>
        ...
        <?php else: ?>
          ...
          <div class="relative" id="profile-menu-wrapper">
            ...
            <div id="profile-dropdown"
              ...
              <!-- Menu items -->
              <div class="py-1">
                <a href="nonresidentProfile.php" role="menuitem"
                   class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-[var(--site-primary-pale)] hover:text-[var(--site-primary-darker)] transition">
                  <i class="fa-solid fa-user w-4 text-gray-400"></i> My Profile
                </a>
                <?php if ($role === 'admin'): ?>
                <a href="adminDashboard.php" role="menuitem"
                   class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-purple-50 hover:text-purple-800 transition">
  ...
  <div id="mobile-sidebar" class="fixed inset-y-0 right-0 w-72 max-w-[85vw] bg-white shadow-2xl transform translate-x-full transition-transform duration-300 z-[70] flex flex-col">
    ...
    <div class="flex-1 overflow-y-auto py-3 px-3 space-y-0.5">
      <a href="#announcements" class="flex items-center gap-3 px-4 py-3 rounded-xl text-[var(--site-primary-dark)] font-medium bg-[var(--site-primary-pale)] transition">
      ...
      <div class="flex-1" data-aos="fade-up">
        ...
        <?php if (str_contains($roleLower, 'resident,business/apartment owner') || str_contains($roleLower, 'business/apartment owner')): ?>
          <p class="text-green-200 text-base max-w-md leading-relaxed">
            Apply for beneficiary programs, request official documents, borrow barangay equipment, post listing and stay updated with announcements and community news. Your portal for convenient barangay services starts here.
          </p>
        <?php else: ?>
          <p class="text-green-200 text-base max-w-md leading-relaxed">
            Apply for beneficiary programs, request official documents, borrow barangay equipment, and stay updated with announcements and community news. Your portal for convenient barangay services starts here.
           </p>
        <?php endif; ?>
        
        <?php if (!$logged_in): ?>
        <div class="mt-6 flex gap-3 flex-wrap">
          <a href="login.php" class="px-6 py-3 bg-green-400 hover:bg-green-300 text-green-950 rounded-lg font-semibold text-sm transition shadow-lg">Login</a>
          <a href="signup/accountCreation.php" class="px-6 py-3 border border-green-400 hover:bg-green-800 text-green-200 rounded-lg font-semibold text-sm transition">Register</a>
        </div>
        <?php endif; ?>
      </div>
...
<section class="max-w-6xl mx-auto px-4 py-16">
  <div class="text-center mb-10">
    ...
    <h2 class="text-3xl font-bold text-green-950 mt-2">What We Offer</h2>
    <p class="text-gray-500 mt-2 text-sm max-w-xl mx-auto">
      Convenient services available to all community members.
    </p>
  </div>

  <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">

    <button type="button" data-url="beneficiaryForm.php" class="service-card rounded-2xl p-7 shadow-sm block text-center group restricted-service" data-service-name="beneficiary programs" data-aos="fade-up" data-aos-delay="100">
      ...
      <p class="font-bold text-gray-800 text-base">Apply for Beneficiary Programs</p>
      <p class="text-gray-400 text-xs mt-2 leading-relaxed">
        Submit applications for barangay assistance programs
      </p>
    </button>

    <button type="button" data-url="documentsForm.php" class="service-card rounded-2xl p-7 shadow-sm block text-center group restricted-service" data-service-name="document requests" data-aos="fade-up" data-aos-delay="200">
      ...
      <p class="font-bold text-gray-800 text-base">Request Documents</p>
      <p class="text-gray-400 text-xs mt-2 leading-relaxed">
        Apply for barangay clearances and certificates online
      </p>
    </button>

    <button type="button" data-url="borrowEquipment.php" class="service-card rounded-2xl p-7 shadow-sm block text-center group restricted-service" data-service-name="equipment borrowing" data-aos="fade-up" data-aos-delay="300">
      ...
      <p class="font-bold text-gray-800 text-base">Borrow Equipment</p>
      <p class="text-gray-400 text-xs mt-2 leading-relaxed">
        Request to borrow barangay equipment for community use
      </p>
    </button>

    <?php $roleLower = strtolower($role); ?>
    <?php if (str_contains($roleLower, 'non-resident,business/apartment owner') || str_contains($roleLower, 'business')): ?>
      <a href="manageList" class="service-card rounded-2xl p-7 shadow-sm block text-center group" data-aos="fade-up" data-aos-delay="400">
        ...
        <p class="font-bold text-gray-800 text-base">Post Listings</p>
        <p class="text-gray-400 text-xs mt-2 leading-relaxed">
          Post business and apartment listings in the area
        </p>
      </a>
    <?php else: ?>
      <a href="../busaptListing" class="service-card rounded-2xl p-7 shadow-sm block text-center group" data-aos="fade-up" data-aos-delay="400">
        <div class="w-14 h-14 mx-auto rounded-2xl bg-orange-50 group-hover:bg-orange-100 flex items-center justify-center transition mb-4">
          <i class="fa-solid fa-list text-2xl text-orange-500"></i>
        </div>
        <p class="font-bold text-gray-800 text-base">Browse Listings</p>
        <p class="text-gray-400 text-xs mt-2 leading-relaxed">
          View available business and apartment listings in the area
        </p>
      </a>
    <?php endif; ?>

    <a href="#announcements" class="service-card rounded-2xl p-7 shadow-sm block text-center group" data-aos="fade-up" data-aos-delay="500">
      ...
      <p class="font-bold text-gray-800 text-base">View Announcements</p>
      <p class="text-gray-400 text-xs mt-2 leading-relaxed">
        Stay updated with the latest barangay announcements and alerts
      </p>
    </a>

    <a href="#announcements" class="service-card rounded-2xl p-7 shadow-sm block text-center group" data-aos="fade-up" data-aos-delay="600">
      ...
      <p class="font-bold text-gray-800 text-base">Community Updates</p>
      <p class="text-gray-400 text-xs mt-2 leading-relaxed">
  ...
  <section class="bg-green-950 text-white py-16 px-4 relative overflow-hidden">
    ...
    <div class="max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-12 relative z-10">
      <div class="flex-1" data-aos="fade-right">
        ...
        <p class="text-green-200 leading-relaxed mb-4"><?= e($siteSettings['our_reach_content'] ?: 'Sumacab Este residents, or even non-residents, can access barangay services through one online portal.') ?></p>
  ...
  <div id="serviceToast" class="access-denied-alert hidden">
    <div class="alert-content">
      <i class="fa-solid fa-lock alert-icon"></i>
      <div class="alert-text">
        <h4>Access Restricted</h4>
        <p id="serviceToastMessage">Only residents can access this service. Please update your profile to Resident to continue.</p>
      </div>
      <button id="serviceToastClose" aria-label="Close notification" class="alert-close">
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const restrictedCards = document.querySelectorAll('.restricted-service');
    ...
    const serviceToastClose = document.getElementById('serviceToastClose');
    let serviceToastTimer;

    function showServiceToast(serviceName) {
      if (!serviceToast || !serviceToastMessage) return;
      serviceToastMessage.textContent = `The ${serviceName} page is available only to residents. Please update your profile to Resident to access this service.`;
    ...
    serviceToastClose?.addEventListener('click', () => {
      serviceToast?.classList.add('hidden');
      clearTimeout(serviceToastTimer);
    });
    ...
    restrictedCards.forEach(card => {
      card.addEventListener('click', (event) => {
        ...
        const serviceName = card.dataset.serviceName || 'selected service';
        showServiceToast(serviceName);
      });
    });
