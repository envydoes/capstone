...
// allow only non-resident to access this page
if (in_array('admin', $roleParts, true)) {
    header('Location: ../admin/adminLanding.php');
    exit;
}
if (in_array('resident', $roleParts, true)) {
    header('Location: ../resident/residentLanding.php');
    exit;
}
...
if ($accId) {
    $colCheck = $conn->query("SHOW COLUMNS FROM tbl_userinfo LIKE 'emergency_contact_relationship'");
    if ($colCheck && $colCheck->num_rows === 0) {
        $conn->query("ALTER TABLE tbl_userinfo ADD COLUMN emergency_contact_relationship VARCHAR(255) DEFAULT '' AFTER emergency_contact");
    }

    $stmt = $conn->prepare('SELECT userID, firstname, middlename, lastname, suffix, family_role, email, phone, birthday, birthplace, gender, civil_status, citizenship, religion, ethnicity, street, barangay, city, province, zip, emergency_contact, emergency_contact_relationship, emergency_phone, health_conditions, employment_status, job_title, monthly_income, voter_id, precinct, years_resident, resident_birth, userStatus, dateRegistered FROM tbl_userinfo WHERE accID = ? LIMIT 1');
    if ($stmt) {
        ...
        if ($row = $result->fetch_assoc()) {
            ...
            $userData['family_role'] = $row['family_role'] ?? '';
            $userData['birthplace'] = $row['birthplace'] ?? '';
            $userData['citizenship'] = $row['citizenship'] ?? 'Filipino';
            $userData['religion'] = $row['religion'] ?? '';
            $userData['ethnicity'] = $row['ethnicity'] ?? '';
            $userData['health_conditions'] = $row['health_conditions'] ?? '';
            $userData['employment_status'] = $row['employment_status'] ?? '';
            $userData['job_title'] = $row['job_title'] ?? '';
            $userData['monthly_income'] = $row['monthly_income'] ?? '';
            $userData['voter_id'] = $row['voter_id'] ?? '';
            $userData['precinct'] = $row['precinct'] ?? '';
            $userData['years_resident'] = $row['years_resident'] ?? '';
            $userData['resident_birth'] = $row['resident_birth'] ?? '';
            $userData['emergency_relation'] = $row['emergency_contact_relationship'] ?? '';
...
<head>
  ...
  <link rel="stylesheet" href="../assets/responsive-global.css">
    <title>My Profile - <?= e($siteSettings['site_title']) ?></title>
      ...
      <section class="rounded-2xl border border-green-100 bg-white p-6 md:p-8 shadow-sm">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div class="flex items-center gap-2">
            <a href="nonresidentLanding.php" class="inline-flex items-center gap-2 text-sm font-semibold text-emerald-700 hover:text-emerald-900 hover:bg-emerald-100 rounded-lg border border-emerald-100 bg-emerald-50 px-3 py-2">
              <i class="fa-solid fa-arrow-left text-xs"></i>
              <span>Back to Portal</span>
            </a>
            <span class="rounded-full bg-green-100 px-3 py-1 text-xs font-semibold text-green-700">Non-Resident</span>
          </div>
        </div>
      </section>
      ...
      <section class="grid gap-6 lg:grid-cols-3">
        <div class="lg:col-span-1 space-y-6">
          <div class="bg-white border border-green-100 rounded-2xl p-6 text-center shadow-sm">
            ...
            <a href="nonresidentEditProfile.php" class="w-full inline-flex items-center justify-center bg-green-700 text-white py-3 rounded-lg font-semibold hover:bg-green-800 transition">Edit Profile</a>
          </div>
