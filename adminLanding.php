...
if (!$isAdminAccess && !in_array($role, $adminRoles, true)) {
    switch ($role) {
        case 'resident':
...
// If this is a limited admin, load which sections they're allowed to see
// so the "Management Services" tiles below can be filtered.
$myPermissions = get_my_permissions($conn); // empty array for full 'admin' (no filtering needed)
...
<body class="bg-gray-50">
  ...
  <header class="w-full h-[68px] border-b border-green-100 flex items-center px-8 bg-white shadow-sm sticky top-0 z-50">
    <div class="flex items-center gap-3">
      <a href="adminDashboard.php" class="flex items-center gap-3">
  ...
  <div id="mobile-sidebar" class="fixed inset-y-0 right-0 w-64 bg-white shadow-2xl transform translate-x-full transition-transform duration-300 z-[70] flex flex-col">
    ...
    <div class="flex-1 overflow-y-auto py-4">
      <nav class="flex flex-col gap-2 px-4">
        <a href="adminDashboard.php" class="px-4 py-3 rounded-xl text-gray-700 font-medium hover:bg-green-50 hover:text-green-700 transition">Dashboard</a>
        <a href="#announcements"     class="px-4 py-3 rounded-xl text-gray-700 font-medium hover:bg-green-50 hover:text-green-700 transition">Announcements</a>
        <a href="#business"          class="px-4 py-3 rounded-xl text-gray-700 font-medium hover:bg-green-50 hover:text-green-700 transition">Business</a>
        <a href="#apartment"         class="px-4 py-3 rounded-xl text-gray-700 font-medium hover:bg-green-50 hover:text-green-700 transition">Apartment</a>
        ...
        <div class="bg-gray-50 rounded-xl p-4 mb-2 border border-gray-100">
          ...
          <div class="flex flex-col gap-1">
            <?php if ($role === 'admin'): ?>
            <a href="adminDashboard.php" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-gray-700 hover:bg-white hover:text-purple-800 transition">
              <i class="fa-solid fa-shield-halved w-4 text-gray-400 text-center"></i> Admin Panel
            </a>
            <?php endif; ?>
            <div class="h-px bg-gray-200 my-1"></div>
            <a href="../logout.php" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-red-600 hover:bg-white hover:text-red-700 transition">
   ...
   <div class="hero-slider" id="heroSlider" style="width: <?= $heroTotalSlides * 100 ?>%;">
    ...
    <div class="relative z-10 max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-12 pt-10 pb-10">
      ...
      <div class="flex-1 grid grid-cols-2 gap-4 w-full max-w-sm" data-aos="fade-up" data-aos-delay="200">
        <div class="bg-black/40 backdrop-blur-md border border-white/20 rounded-2xl p-5 text-center shadow-lg"><p class="text-3xl font-bold text-green-400">1,200+</p><p class="text-xs text-green-200 mt-1 uppercase tracking-wider">Residents Served</p></div>
...
<section class="max-w-6xl mx-auto px-4 py-16">
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
    ...
    <?php if (tileAllowed('dashboard', $role, $myPermissions)): ?>
    <a href="adminDashboard.php" class="service-card rounded-2xl p-7 shadow-sm block text-center group" data-aos="fade-up" data-aos-delay="50">
      <div class="w-14 h-14 mx-auto rounded-2xl bg-emerald-50 group-hover:bg-emerald-100 flex items-center justify-center transition mb-4">
        <i class="fa-solid fa-chart-line text-2xl text-emerald-600"></i>
      </div>
      <p class="font-bold text-gray-800 text-base">Admin Panel</p>
      <p class="text-gray-400 text-xs mt-2 leading-relaxed">
        View the admin panel and system metrics.
      </p>
    </a>
    <?php endif; ?>

    <?php if (tileAllowed('manage_beneficiaries', $role, $myPermissions)): ?>
    ...
    <?php if (tileAllowed('manage_documents', $role, $myPermissions)): ?>
    ...
    <?php if (tileAllowed('manage_borrowing', $role, $myPermissions)): ?>
    ...
    <?php if (tileAllowed('manage_listings', $role, $myPermissions)): ?>
    ...
    <?php if (tileAllowed('manage_announcements', $role, $myPermissions)): ?>
